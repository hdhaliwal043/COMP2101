...
#assigning given commands to variables
hostname=$(hostname)
lanAddress=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
lanHostname=$(getent hosts $lanAdress | awk '{print $2}')
...
#assigning the command of router address to a variable
routerAddress=$(ip r | awk '/via /{gsub(/\/.*/,"");print $3}')

#   Router Hostname : router-name-from-hosts-file
#assigning the command of router hostto a variable
...
#displaying variable with the ouput of commands stored in them
cat<<EOF
Host name: $hostname
